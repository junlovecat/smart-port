...
craneside_upper=pygame.image.load('craneside.png').convert_alpha()
craneside_upper=pygame.transform.scale(craneside_upper,cmtopixelset((1.46,0.65)))
# craneside_upper (크레인 옆에 붙어있는거? 윗부분) 선언 및 이미지 로드, 배경색 투명화, 크기 변경

craneside_lower=pygame.image.load('craneside.png').convert_alpha()
craneside_lower=pygame.transform.scale(craneside_lower,cmtopixelset((1.46,0.65)))
craneside_lower=pygame.transform.rotate(craneside_lower,180)

# 상자가 배 1에서 플랫폼 1로 이동하는데 사용됨
boxonetick=0 # 움직인 수를 저장
boxonemover=list(reversed([(xx,cmtopixel(2.3)) for xx in numpy.arange(215.43307087,550,0.1)]))
# 움직이는 위치를 저장
boxone=chooseabox() # 사용되는 상자 초기값 세팅

# 상자가 플랫폼 2에서 배 2로 이동하는데 사용됨
boxtwotwotick=0
boxtwotwomover=[(-100,-100) for xx in range(1000)]+[(xx,cmtopixel(5.6)) for xx in numpy.arange(215.43307087,550,0.1)]
boxtwotwo=chooseabox()

# 상자가 플랫폼 2에서 플랫폼 3으로, 플랫폼 3에서 배 3으로 이동하는데 사용됨
boxtwothreetick=0
boxtwothreemover=[(xx,cmtopixel(5.6)) for xx in numpy.arange(215,525,0.1)]+[(525,xx) for xx in numpy.arange(cmtopixel(5.6),cmtopixel(8),0.1)]+[(xx,cmtopixel(8)) for xx in numpy.arange(525,580,0.1)]+[(580,xx) for xx in numpy.arange(cmtopixel(8),cmtopixel(10.1),0.1)]+[(xx,cmtopixel(10.1)) for xx in numpy.arange(580,550,0.1)]
boxtwothree=chooseabox()

# 상자가 플랫폼 3에서 플랫폼 2로, 플랫폼 2에서 배 2로 이동하는데 사용됨
boxthreetwotick=0
boxthreetwomover=[(-100,-100) for xx in range(1050)]+[(xx,cmtopixel(8)) for xx in numpy.arange(215,525,0.1)]+[(525,xx) for xx in list(reversed(numpy.arange(cmtopixel(5.6),cmtopixel(8),0.1)))]+[(xx,cmtopixel(5.6)) for xx in numpy.arange(525,580,0.1)]+[(xx,cmtopixel(5.6)) for xx in numpy.arange(580,550,0.1)]
boxthreetwo=chooseabox()

# 상자가 플랫폼 3에서 배 3으로 이동하는데 사용됨
boxthreetick=0
boxthreemover=[(-100,-100) for xx in range(100)]+[(xx,cmtopixel(8)) for xx in numpy.arange(215.43307087,580,0.1)]+[(580,xx) for xx in numpy.arange(cmtopixel(8),cmtopixel(10.1),0.1)]+[(xx,cmtopixel(10.1)) for xx in numpy.arange(580,550,0.1)]
boxthree=chooseabox()

#상자가 배 4에서 플랫폼 4로 이동하는데 사용됨
boxfourtick=0
boxfourmover=list(reversed([(-100,-100) for xx in range(500)]+[(xx,cmtopixel(11.2)) for xx in numpy.arange(215.43307087,580,0.1)]+[(580,xx) for xx in numpy.arange(cmtopixel(11.2),cmtopixel(13.5),0.1)]+[(xx,cmtopixel(13.5)) for xx in numpy.arange(580,550,0.1)]))
boxfour=chooseabox()
...
while(1):
    ...
    # 크레인 4
    cranesides_upper=[(17.4,1.4),(17.4,4.7)]
    for x in cranesides_upper:
        window.blit(craneside_upper,cmtopixelset(x))
